...
%% Problem 4: Kinematic EOM for quaternion
% See quaternionEOM under Useful Functions.

%% Problem 5: Kinematic EOM for DCM
% See dcmEOM under Unseful Functions.

%% Problem 6: Integrate kinematic and Euler eqns for spacecraft (quaternions)
% Define initial conditions and output vector
...
%% Problem 6: Integrate kinematic and Euler eqns for spacecraft (DCM)
% Define initial conditions and output vector
wx0 = deg2rad(25); wy0 = deg2rad(-20); wz0 = deg2rad(-12);  % initial rotation rates (deg/s)
y0d = zeros(12,1);
y0d(1) = 1; y0d(5) = 1; y0d(9) = 1; % zero initial angle
y0d(10) = wx0;   % initial rotation rate about x axis (rad/s)
y0d(11) = wy0;   % initial rotation rate about y axis (rad/s)
y0d(12) = wz0;   % initial rotation rate about z axis (rad/s)

% Simulation outputs - quaternion
[t, y_out] = ode113(@eulerWithDCM, [0:tstep:t_period]' , y0d, options);
r11 = y_out(:,1); r12 = y_out(:,2); r13 = y_out(:,3);
r21 = y_out(:,4); r22 = y_out(:,5); r23 = y_out(:,6);
r31 = y_out(:,7); r32 = y_out(:,8); r33 = y_out(:,9);
wx = y_out(:,10); wy = y_out(:,11); wz = y_out(:,12);

% Plot results
figure(); title('Angular Velocity vs. Time - DCM Implementation'); hold on; grid on;
plot(t,wx); plot(t,wy); plot(t,wz); legend('wx', 'wy', 'wz');
xlabel('t (s)'); ylabel('Rad/s'); hold off;

figure(); sgtitle('Attitude vs. Time - DCM Implementation')
subplot(3,3,1); plot(t, r11, 'b-'); xlabel('t (s)'); ylabel('R11'); grid on;
subplot(3,3,2); plot(t, r12, 'b--'); xlabel('t (s)'); ylabel('R12'); grid on;
subplot(3,3,3); plot(t, r13, 'b:'); xlabel('t (s)'); ylabel('R13');grid on;
subplot(3,3,4); plot(t, r21, 'r-'); xlabel('t (s)'); ylabel('R21');grid on;
subplot(3,3,5); plot(t, r22, 'r--'); xlabel('t (s)'); ylabel('R22');grid on;
subplot(3,3,6); plot(t, r23, 'r:'); xlabel('t (s)'); ylabel('R23');grid on;
subplot(3,3,7); plot(t, r31, 'm-'); xlabel('t (s)'); ylabel('R31');grid on;
subplot(3,3,8); plot(t, r32, 'm--'); xlabel('t (s)'); ylabel('R32');grid on;
subplot(3,3,9); plot(t, r33, 'm:');xlabel('t (s)'); ylabel('R33');grid on;
